...
public class FarmersDelightClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        ItemProperties.register(ModItems.SKILLET.get(), new ResourceLocation("cooking"),
                (stack, world, entity, s) -> stack.getTagElement("Cooking") != null ? 0 : 1);
        ModNetworking.initClient();
        RecipeCategories.init();

        // Obscure Fabric event to the rescue!
        ClientPreAttackCallback.EVENT.register((client, player, clickCount) -> {
            if (player != null && !player.isSpectator() && player.isUsingItem() && player.getUseItem().getItem() instanceof SkilletItem && clickCount != 0 && player.getUseItem().getOrCreateTag().getLong("FlipTimeStamp") == 0L) {

...
public class DecompositionViewRecipe implements IEivViewRecipe {
    ...
    public void renderRecipe(RecipeViewScreen screen, GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTicks) {
        ...
        if (ClientRenderUtils.isCursorInsideBounds(iconX, iconY, 11, 11, mouseX, mouseY)) {
            guiGraphics.renderTooltip(Minecraft.getInstance().font, LIGHT_TOOLTIP, mouseX, mouseY);
        } else if (ClientRenderUtils.isCursorInsideBounds(iconX+12, iconY, 11, 11, mouseX, mouseY)) {
            guiGraphics.renderTooltip(Minecraft.getInstance().font, FLUID_TOOLTIP, mouseX, mouseY);
        } else if (ClientRenderUtils.isCursorInsideBounds(iconX+27, iconY, 11, 11, mouseX, mouseY)) {
            guiGraphics.renderTooltip(Minecraft.getInstance().font, ACCELERATORS_TOOLTIP, mouseX, mouseY);
        }
    }

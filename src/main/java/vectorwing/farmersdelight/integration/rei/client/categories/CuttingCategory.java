...
public class CuttingCategory implements DisplayCategory<CuttingDisplay> {
    private static final Identifier BACKGROUND = Identifier.fromNamespaceAndPath(FarmersDelight.MODID, "textures/gui/jei/cutting_board.png");
    ...
    public Component getTitle() {
        return Component.translatable("jei.farmersdelight.cutting");
    }
    ...
    public List<Widget> setupDisplay(CuttingDisplay display, Rectangle bounds) {
        ...
        for (int i = 0; i < size; ++i) {
            ...
            EntryIngredient ingredient = output.getFirst();
            if (!ingredient.isEmpty()) {
                float chance = output.getSecond();
                if (chance < 1.0F) {
                    ingredient = ingredient.map(stack -> stack.copy().tooltip(TextUtils.JEI("chance", chance < 0.01 ? "<1" : (int) (chance * 100))
                            .withStyle(ChatFormatting.GOLD)));
                }
            }

            widgets.add(Widgets.createTexturedWidget(BACKGROUND, startPoint.x + OUTPUT_GRID_X + xOffset - 1, startPoint.y + OUTPUT_GRID_Y + yOffset - 1, output.getSecond() < 1 ?  18 : 0, 58, 18, 18));

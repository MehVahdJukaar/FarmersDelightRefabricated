...
public class SoilUtils {
    public static boolean isAbleToPlaceRichSoil(Block block) {
        if (block.builtInRegistryHolder().is(FDRefabricatedTags.Blocks.DOES_NOT_SURVIVE_RICH_SOIL))
            return false;

        if (block.builtInRegistryHolder().is(FDRefabricatedTags.Blocks.SURVIVES_RICH_SOIL))
            return true;
    ...
    public static boolean isAbleToPlaceRichSoilFarmland(Block block) {
        if (block.builtInRegistryHolder().is(FDRefabricatedTags.Blocks.DOES_NOT_SURVIVE_RICH_SOIL_FARMLAND))
            return false;

        if (block.builtInRegistryHolder().is(FDRefabricatedTags.Blocks.SURVIVES_RICH_SOIL_FARMLAND))
            return true;

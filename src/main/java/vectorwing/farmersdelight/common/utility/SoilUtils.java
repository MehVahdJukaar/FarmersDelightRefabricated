...
public class SoilUtils {
    public static boolean isAbleToPlaceRichSoil(Block block) {
        if (BuiltInRegistries.BLOCK.wrapAsHolder(block).is(ModTags.DOES_NOT_SURVIVE_RICH_SOIL))
            return false;

        if (BuiltInRegistries.BLOCK.wrapAsHolder(block).is(ModTags.SURVIVES_RICH_SOIL_FARMLAND))
            return true;
    ...
    public static boolean isAbleToPlaceRichSoilFarmland(Block block) {
        if (BuiltInRegistries.BLOCK.wrapAsHolder(block).is(ModTags.DOES_NOT_SURVIVE_RICH_SOIL_FARMLAND))
            return false;

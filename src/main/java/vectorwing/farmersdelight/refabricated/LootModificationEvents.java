...
public class LootModificationEvents {
	...
	private static void chestLoot(ResourceKey<LootTable> key, LootTable.Builder tableBuilder, LootTableSource source, HolderLookup.Provider registries) {
		...
		if (key == BuiltInLootTables.ABANDONED_MINESHAFT)
			tableBuilder.withPool(LootPool.lootPool().add(lootTableReference(registries, (FD_ABANDONED_MINESHAFT))));

		if (key == BuiltInLootTables.BASTION_HOGLIN_STABLE)
			tableBuilder.withPool(LootPool.lootPool().add(lootTableReference(registries, (FD_BASTION_HOGLIN_STABLE))));

		if (key == BuiltInLootTables.BASTION_TREASURE)
			tableBuilder.withPool(LootPool.lootPool().add(lootTableReference(registries, (FD_BASTION_TREASURE))));

		if (key == BuiltInLootTables.END_CITY_TREASURE)
			tableBuilder.withPool(LootPool.lootPool().add(lootTableReference(registries, (FD_END_CITY_TREASURE))));

		if (key == BuiltInLootTables.PILLAGER_OUTPOST)
			tableBuilder.withPool(LootPool.lootPool().add(lootTableReference(registries, (FD_PILLAGER_OUTPOST))));

		if (key == BuiltInLootTables.RUINED_PORTAL)
			tableBuilder.withPool(LootPool.lootPool().add(lootTableReference(registries, (FD_RUINED_PORTAL))));

		if (key == BuiltInLootTables.SHIPWRECK_SUPPLY)
			tableBuilder.withPool(LootPool.lootPool().add(lootTableReference(registries, (FD_SHIPWRECK_SUPPLY))));

		if (key == BuiltInLootTables.SIMPLE_DUNGEON)
			tableBuilder.withPool(LootPool.lootPool().add(lootTableReference(registries, (FD_SIMPLE_DUNGEON))));

		if (key == BuiltInLootTables.VILLAGE_BUTCHER)
			tableBuilder.withPool(LootPool.lootPool().add(lootTableReference(registries, (FD_VILLAGE_BUTCHER))));

		if (key == BuiltInLootTables.VILLAGE_DESERT_HOUSE)
			tableBuilder.withPool(LootPool.lootPool().add(lootTableReference(registries, FD_VILLAGE_DESERT_HOUSE)));

		if (key == BuiltInLootTables.VILLAGE_PLAINS_HOUSE)
			tableBuilder.withPool(LootPool.lootPool().add(lootTableReference(registries, FD_VILLAGE_PLAINS_HOUSE)));

		if (key == BuiltInLootTables.VILLAGE_SAVANNA_HOUSE)
			tableBuilder.withPool(LootPool.lootPool().add(lootTableReference(registries, FD_VILLAGE_SAVANNA_HOUSE)));

		if (key == BuiltInLootTables.VILLAGE_SNOWY_HOUSE)
			tableBuilder.withPool(LootPool.lootPool().add(lootTableReference(registries, FD_VILLAGE_SNOWY_HOUSE)));

		if (key == BuiltInLootTables.VILLAGE_TAIGA_HOUSE)
			tableBuilder.withPool(LootPool.lootPool().add(lootTableReference(registries, FD_VILLAGE_TAIGA_HOUSE)));
	}

	private static LootPoolEntryContainer lootTableReference(HolderLookup.Provider registries, ResourceKey<LootTable> lootTable) {
		return NestedLootTable.lootTableReference(registries.getOrThrow(lootTable)).build();
	}
    ...
    private static void scavengingLoot(ResourceKey<LootTable> key, LootTable.Builder tableBuilder, LootTableSource source, HolderLookup.Provider registries) {
        ...
        if (key == BLOCKS_PUMPKIN) {
            tableBuilder.modifyPools(builder ->
                    builder.when(MatchTool.toolMatches(ItemPredicate.Builder.item().of(registries.lookupOrThrow(Registries.ITEM), ModTags.Items.KNIVES)).invert()
    ...
    private static void slicingLoot(ResourceKey<LootTable> key, LootTable.Builder tableBuilder, LootTableSource source, HolderLookup.Provider registries) {
		...
		if (key == BLOCKS_PUMPKIN_PIE)
			pastrySlicing(tableBuilder, ModBlocks.PUMPKIN_PIE.get(), ModItems.PUMPKIN_PIE_SLICE.get(), PieBlock.BITES, 4, registries);
    ...
    public static void strawChance02(LootTable.Builder tableBuilder, HolderLookup.Provider registries) {
        tableBuilder.withPool(LootPool.lootPool().add(LootItem.lootTableItem(ModItems.STRAW.get())
                .when(LootItemRandomChanceCondition.randomChance(0.3F)
                        .and(MatchTool.toolMatches(ItemPredicate.Builder.item().of(registries.lookupOrThrow(Registries.ITEM), ModTags.Items.STRAW_HARVESTERS))))));
    }
